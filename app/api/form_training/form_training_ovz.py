...
form_training_ovz = Blueprint("form-training-ovz", __name__)


@form_training_ovz.route("/", methods=["GET"])
def get_questions_ovz():
    ...
    try:
        ...
        cur.execute("""
            ...
            JOIN practical.teacher_training_testing_answer a
                ON q.id = a.id_quest
            WHERE q.id_block = 1 AND a.display IS TRUE AND q.display IS TRUE
            ORDER BY q.id, a.id
        """)
        rows = cur.fetchall()
        cur.close()
...
# POST
@form_training_ovz.route("/save", methods=["POST"])
def post_answer_ovz():
