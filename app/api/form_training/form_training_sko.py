...
form_training_sko = Blueprint("form-training-sko", __name__)


@form_training_sko.route("/", methods=["GET"])
def get_questions_sko():
    ...
    try:
        ...
        cur.execute("""
            ...
            JOIN practical.teacher_training_testing_answer a
                ON q.id = a.id_quest
            WHERE q.id_block = 4 AND a.display IS TRUE AND q.display IS TRUE
            ORDER BY q.id, a.id
        """)
...
# POST
@form_training_sko.route("/save", methods=["POST"])
def post_answer_sko():
